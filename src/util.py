...
def get_templates(args, src, tgt, tgt_code):
    ...
    if n_shot == 0:
        ...
        templates_gpt = [f"The word '{{}}' in {tgt} is:", #0,37
                        ...
                        f"The {src} word {{}} in {tgt} is:", #3,40 xglm-564M-Best (0-shot) xglm-1.7B-Best (0-shot) xglm-2.9B-Best (0-shot) xglm-4.5B-Best (0-shot) xglm-7.5B-Best (0-shot) llama-7B-Best (0-shot) Llama-2-7b-Best (0-shot) Llama-2-13b-Best (0-shot)
        ...
        templates_gpt = [f"Translate from {src} to {tgt}: " + f"{{}}->{{}} "*n_shot + f"{{}}->", #0,12
                        ...
                        f"The {src} word {{}} in {tgt} is {{}}. "*n_shot + f"The {src} word {{}} in {tgt} is", #6,18 xglm-1.7B-Best (5-shot) xglm-2.9B-Best (5-shot) xglm-4.5B-Best (5-shot) xglm-7.5B-Best (5-shot) mGPT-Best (5-shot) flan-t5-xxl-BEST (5-shot) mt0-base-Best (5-shot) Llama-2-7b-Best (5-shot) 
                        ...
                        f"The {src} word '{{}}' in {tgt} is {{}}. "*n_shot + f"The {src} word '{{}}' in {tgt} is", #14,26 llama-7B-Best (5-shot) llama-13B-Best (5-shot) Llama-2-13b-Best (5-shot)
